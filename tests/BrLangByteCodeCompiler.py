...
class Parser:
  ...
  def class_def(self):
    ...
    pos_end = self.current_tok.pos_end.copy()
    self.advance(res)
    return res.success(ClassDefNode(class_name_tok, methods, pos_start, pos_end))
...
def isLangExtension(x):
  if x.endswith(".br"):
    ...
    print("Extension Error: The extension needs be '.br', not '." + x.split('.')[1] + "'")
...
def main():
  try:
    ...
    bytess = compile(f'from BrLangByteCodeCompiler import run\nrun("""{fn}""", """{code}""")', '<string>', 'exec')

    disassembler = '\n'.join([line for line in dis.Bytecode(bytess).dis().splitlines()])

    open(isLangExtension(fn).replace('.br', '.bro'), "w").write(''.join(f'\\{ord(c):03o}' for c in disassembler))
  except:
    try:
